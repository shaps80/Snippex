 ...
 * 
 * Step 3:
 * Replace your NSLog statements with SLLog statements according to the severity of the message.
 * 
 * NSLog(@"Fatal error, no dohickey found!"); -> SLLogError(@"Fatal error, no dohickey found!");
 * 
 * SLLog works exactly the same as NSLog.
 ...
 * 
 * if (logFlagForThisLogMsg & slLogLevel) { execute log message }
 * 
 ...
 * 
 * (If the compiler sees slLogLevel declared as a constant, the compiler simply checks to see if the 'if' statement
 ...
 * 
 * The SLLogMessage object includes a 'tag' ivar that may be used for a variety of purposes.
 ...
 * 
 * static const int slLogLevel = LOG_FLAG_ERROR | LOG_FLAG_INFO;
 * 
 * Flags may also be consulted when writing custom log formatters,
 * as the SLLogMessage class captures the individual flag that caused the log message to fire.
 * 
 * -- Levels --
 * 
 ...
 *         SLLogVerbose(@"sprocket: %@", [sprocket description])
 * }
 * 
 * -- Async --
 * 
...
#define LOG_LEVEL_DEBUG   (LOG_LEVEL_VERBOSE | LOG_FLAG_DEBUG)

#define LOG_ERROR   (slLogLevel & LOG_FLAG_ERROR)
#define LOG_WARN    (slLogLevel & LOG_FLAG_WARN)
#define LOG_INFO    (slLogLevel & LOG_FLAG_INFO)
#define LOG_VERBOSE (slLogLevel & LOG_FLAG_VERBOSE)
#define LOG_DEBUG   (slLogLevel & LOG_FLAG_DEBUG)
...
#define SLLogError(frmt, ...)   LOG_OBJC_MAYBE(LOG_ASYNC_ERROR,   slLogLevel, LOG_FLAG_ERROR,   0, frmt, ##__VA_ARGS__)
#define SLLogWarn(frmt, ...)    LOG_OBJC_MAYBE(LOG_ASYNC_WARN,    slLogLevel, LOG_FLAG_WARN,    0, frmt, ##__VA_ARGS__)
#define SLLogInfo(frmt, ...)    LOG_OBJC_MAYBE(LOG_ASYNC_INFO,    slLogLevel, LOG_FLAG_INFO,    0, frmt, ##__VA_ARGS__)
#define SLLogVerbose(frmt, ...) LOG_OBJC_MAYBE(LOG_ASYNC_VERBOSE, slLogLevel, LOG_FLAG_VERBOSE, 0, frmt, ##__VA_ARGS__)
#define SLLogDebug(frmt, ...)   LOG_OBJC_MAYBE(LOG_ASYNC_ERROR,   slLogLevel, LOG_FLAG_DEBUG,   0, frmt, ##__VA_ARGS__)

#define SLLogCError(frmt, ...)   LOG_C_MAYBE(LOG_ASYNC_ERROR,   slLogLevel, LOG_FLAG_ERROR,   0, frmt, ##__VA_ARGS__)
#define SLLogCWarn(frmt, ...)    LOG_C_MAYBE(LOG_ASYNC_WARN,    slLogLevel, LOG_FLAG_WARN,    0, frmt, ##__VA_ARGS__)
#define SLLogCInfo(frmt, ...)    LOG_C_MAYBE(LOG_ASYNC_INFO,    slLogLevel, LOG_FLAG_INFO,    0, frmt, ##__VA_ARGS__)
#define SLLogCVerbose(frmt, ...) LOG_C_MAYBE(LOG_ASYNC_VERBOSE, slLogLevel, LOG_FLAG_VERBOSE, 0, frmt, ##__VA_ARGS__)

/**
 * The THIS_FILE macro gives you an NSString of the file name.
 * For simplicity and clarity, the file name does not include the full path or file extension.
 * 
 * For example: SLLogWarn(@"%@: Unable to find thingy", THIS_FILE) -> @"MyViewController: Unable to find thingy"
**/

NSString *SLExtractFileNameWithoutExtension(const char *filePath, BOOL copy);

#define THIS_FILE (SLExtractFileNameWithoutExtension(__FILE__, NO))

/**
 * The THIS_METHOD macro gives you the name of the current objective-c method.
 * 
 * For example: SLLogWarn(@"%@ - Requires non-nil strings", THIS_METHOD) -> @"setMake:model: requires non-nil strings"
 * 
...
+ (void)addLogger:(id <SLLogger>)logger;
+ (void)removeLogger:(id <SLLogger>)logger;
...
- (void)setLogFormatter:(id <SLLogFormatter>)formatter;
 ...
 * 
 * This allows invocations of SLLog's flushLog method to be propogated to loggers that need it.
 * 
 * Note that SLLog's flushLog method is invoked automatically when the application quits,
...
/**
 * Formatters may optionally be added to any logger.
 * This allows for increased flexibility in the logging enviroslent.
...
**/
- (void)didAddToLogger:(id <SLLogger>)logger;
- (void)willRemoveFromLogger:(id <SLLogger>)logger;
...
@protocol SLRegisteredDynamicLogging
...
/**
 * The SLLogMessage class encapsulates information about the log message.
 * If you write custom loggers or formatters, you will be dealing with objects of this class.
**/

enum {
	SLLogMessageCopyFile     = 1 << 0,
	SLLogMessageCopyFunction = 1 << 1,
};
typedef int SLLogMessageOptions;

@interface SLLogMessage : NSObject
{
...
@public
	...
	// For 3rd party extensions that manually create SLLogMessage instances.
	SLLogMessageOptions options;
}
 ...
 * Options is a bitmask which supports SLLogMessageCopyFile and SLLogMessageCopyFunction.
**/
- (id)initWithLogMsg:(NSString *)logMsg
            ...
            function:(const char *)function
                line:(int)line
                 tag:(id)tag
             options:(SLLogMessageOptions)optionsMask;
...
/**
 * The SLLogger protocol specifies that an optional formatter can be added to a logger.
...
@interface SLAbstractLogger : NSObject <SLLogger>
{
	id <SLLogFormatter> formatter;
