...
@implementation SLDispatchQueueLogFormatter
{
...
{
	// As per the SLLogFormatter contract, this method is always invoked on the same thread/dispatch_queue
...
- (NSString *)formatLogMessage:(SLLogMessage *)logMessage
{
	...
	return [NSString stringWithFormat:@"%@ | %@ | %i | %@ | %s | %@",
			timestamp,
			queueThreadLabel,
