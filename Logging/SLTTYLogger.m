...
@interface SLTTYLoggerColorProfile : NSObject {
@public
	int mask;
	int context;
...
static SLTTYLogger *sharedInstance;
...
+ (NSUInteger)codeIndexForColor:(OSColor *)inColor
{
	...
	for (OSColor *color in colors)
	{
		...
		NSLogVerbose(@"SLTTYLogger: %3lu : %.3f,%.3f,%.3f & %.3f,%.3f,%.3f = %.6f",
		...
		if (distance < lowestDistance)
		{
			...
			NSLogVerbose(@"SLTTYLogger: New best index = %lu", (unsigned long)bestIndex);
		}
...
+ (void)initialize
{
	...
	if (!initialized)
	{
		...
		NSLogInfo(@"SLTTYLogger: isaColorTTY = %@", (isaColorTTY ? @"YES" : @"NO"));
		NSLogInfo(@"SLTTYLogger: isaColor256TTY: %@", (isaColor256TTY ? @"YES" : @"NO"));
		NSLogInfo(@"SLTTYLogger: isaXcodeColorTTY: %@", (isaXcodeColorTTY ? @"YES" : @"NO"));
		
		sharedInstance = [[SLTTYLogger alloc] init];
	}
}

+ (SLTTYLogger *)sharedInstance
{
...
- (BOOL)colorsEnabled
{
	// The design of this method is taken from the SLAbstractLogger implementation.
	// For extensive documentation please refer to the SLAbstractLogger implementation.
...
- (void)setForegroundColor:(OSColor *)txtColor backgroundColor:(OSColor *)bgColor forFlag:(int)mask context:(int)ctxt
{
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		SLTTYLoggerColorProfile *newColorProfile =
		    [[SLTTYLoggerColorProfile alloc] initWithForegroundColor:txtColor
		                                             backgroundColor:bgColor
		                                                        flag:mask
		                                                     context:ctxt];
		
		NSLogInfo(@"SLTTYLogger: newColorProfile: %@", newColorProfile);
		
		NSUInteger i = 0;
		for (SLTTYLoggerColorProfile *colorProfile in colorProfilesArray)
		{
	...
	// The design of the setter logic below is taken from the SLAbstractLogger implementation.
	// For documentation please refer to the SLAbstractLogger implementation.
...
- (void)logMessage:(SLLogMessage *)logMessage
{
	...
	if (logMsg)
	{
		...
		SLTTYLoggerColorProfile *colorProfile = nil;
		...
		if (colorsEnabled)
		{
			...
			if (colorProfile == nil)
			{
				for (SLTTYLoggerColorProfile *cp in colorProfilesArray)
				{
...
@implementation SLTTYLoggerColorProfile
...
- (id)initWithForegroundColor:(OSColor *)fgColor backgroundColor:(OSColor *)bgColor flag:(int)aMask context:(int)ctxt
{
	if ((self = [super init]))
	{
		...
		if (fgColor)
		{
			[SLTTYLogger getRed:&r green:&g blue:&b fromColor:fgColor];
		...
		if (bgColor)
		{
			[SLTTYLogger getRed:&r green:&g blue:&b fromColor:bgColor];
		...
		if (fgColor && isaColorTTY)
		{
			...
			fgCodeIndex = [SLTTYLogger codeIndexForColor:fgColor];
		...
		if (bgColor && isaColorTTY)
		{
			...
			bgCodeIndex = [SLTTYLogger codeIndexForColor:bgColor];
...
- (NSString *)description
{
	return [NSString stringWithFormat:
			@"<SLTTYLoggerColorProfile: %p mask:%i ctxt:%i fg:%u,%u,%u bg:%u,%u,%u fgCode:%@ bgCode:%@>",
