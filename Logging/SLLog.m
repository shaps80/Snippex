...
// We probably shouldn't be using SLLog() statements within the SLLog implementation.
...
#define NSLogDebug(frmt, ...) do{ if(SL_DEBUG) NSLog((frmt), ##__VA_ARGS__); } while(0)
...
// The "global logging queue" refers to [SLLog loggingQueue].
...
@interface SLLoggerNode : NSObject {
@public 
	id <SLLogger> logger;	
	dispatch_queue_t loggerQueue;
}

+ (SLLoggerNode *)nodeWithLogger:(id <SLLogger>)logger loggerQueue:(dispatch_queue_t)loggerQueue;

@end


@interface SLLog (PrivateAPI)

+ (void)lt_addLogger:(id <SLLogger>)logger;
+ (void)lt_removeLogger:(id <SLLogger>)logger;
+ (void)lt_removeAllLoggers;
+ (void)lt_log:(SLLogMessage *)logMessage;
+ (void)lt_flush;
...
+ (void)initialize
{
	...
	if (!initialized)
	{
		...
		NSLogDebug(@"SLLog: Using grand central dispatch");
		...
		NSLogDebug(@"SLLog: numProcessors = %u", numProcessors);
     ...
     format:(NSString *)format, ...
{
	...
	if (format)
	{
		...
		SLLogMessage *logMessage = [[SLLogMessage alloc] initWithLogMsg:logMsg
		                                                          level:level
		                                                           flag:flag
		                                                        context:context
		                                                           file:file
...
{
	SEL getterSel = @selector(slLogLevel);
	SEL setterSel = @selector(slSetLogLevel:);
	...
	// 
	// The method +[SLLog isRegisteredClass] will crash a project when using it with ARC + Simulator.
...
+ (NSArray *)registeredClasses
{
	...
	// We can now loop through the classes, and test each one to see if it is a SLLogging class.
...
+ (void)lt_addLogger:(id <SLLogger>)logger
{
	...
	SLLoggerNode *loggerNode = [SLLoggerNode nodeWithLogger:logger loggerQueue:loggerQueue];
...
+ (void)lt_removeLogger:(id <SLLogger>)logger
{
	...
	SLLoggerNode *loggerNode = nil;
	
	for (SLLoggerNode *node in loggers)
	{
	...
	if (loggerNode == nil)
	{
		NSLogDebug(@"SLLog: Request to remove logger which wasn't added");
		return;
	}
...
+ (void)lt_removeAllLoggers
{
	...
	for (SLLoggerNode *loggerNode in loggers)
	{
...
@implementation SLLoggerNode

- (id)initWithLogger:(id <SLLogger>)aLogger loggerQueue:(dispatch_queue_t)aLoggerQueue
{
	if ((self = [super init]))
	{
		logger = aLogger;
...
+ (SLLoggerNode *)nodeWithLogger:(id <SLLogger>)logger loggerQueue:(dispatch_queue_t)loggerQueue
{
	return [[SLLoggerNode alloc] initWithLogger:logger loggerQueue:loggerQueue];
}
             ...
             options:(SLLogMessageOptions)optionsMask
{
	if ((self = [super init]))
	{
		...
		if (options & SLLogMessageCopyFile)
			file = dd_str_copy(aFile);
		else
			file = (char *)aFile;
		
		if (options & SLLogMessageCopyFunction)
...
- (NSString *)fileName
{
	return SLExtractFileNameWithoutExtension(file, NO);
}
...
- (void)dealloc
{
	if (file && (options & SLLogMessageCopyFile))
		free(file);
	
	if (function && (options & SLLogMessageCopyFunction))
		free(function);
...
@implementation SLAbstractLogger
...
- (id <SLLogFormatter>)logFormatter
{
	...
	//
	// SLLogVerbose(@"log msg 1");
	// SLLogVerbose(@"log msg 2");
	// [logger setFormatter:myFormatter];
	// SLLogVerbose(@"log msg 3");
	//
	...
	// IMPORTANT NOTE:
	// 
	// Methods within the SLLogger implementation MUST access the formatter ivar directly.
	...
	dispatch_queue_t globalLoggingQueue = [SLLog loggingQueue];
